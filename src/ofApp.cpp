...
void ofApp::setup() {
    ...
    for (int i = 0; i <(int) dir.size(); i++) {
        ...
        player[i].loadSound(dir.getPath(i));
    }
...
void ofApp::update() {
    ...
    if (playSeq.getbActive()) {
        if (bLoadSeq && !player[randomSequence[playerCounter]].getIsPlaying()){
            playSequence();
        }
    ...
    //when the sequence has finished just RESET
    if (playerCounter == counter - 1 && !player[randomSequence[playerCounter]].getIsPlaying()) {
...
void ofApp::draw() {
    ...
    if (bGameover) {
        ...
        winMessage.loadFont("ArialBold.ttf", 100);
        winMessage.drawString(message, (ofGetWidth() - winMessage.stringWidth(message))/2, (ofGetHeight()/2) );
        winMessage.loadFont("ArialBold.ttf", 20);
        winMessage.drawString("press spacebar", (ofGetWidth() - winMessage.stringWidth("press spacebar"))/2, (ofGetHeight()/2 + 40) );
...
void ofApp::drawDebug(bool b){
    ...
    if (b) {
        ...
        if (playerCounter>0) {
            ofDrawBitmapString("is playing? " + ofToString(player[randomSequence[playerCounter]].getIsPlaying()), 20, 180);
        }
